  ...
  const Salary = Object.values(OldUsers.map(x => x.salary));

  const arr = Object.values(OldUsers.map(x => x.fullName));

  for (var i = 0; i < arr.length; i++) {
      let strarr = arr[i].split("_");
      strarr[0] = strarr[0].charAt(0).toUpperCase() + strarr[0].slice(1);
      strarr[1] = strarr[1].charAt(0).toUpperCase() + strarr[1].slice(1);
      arr[i] = strarr[0] + " " + strarr[1];
    }
